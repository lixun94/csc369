...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		// DIRTY_BIT set not dirty and ONSWAP_BIT on swap
...
char *find_physpage(addr_t vaddr, char type) {
	...
	// create a new pagetable if valid
	...
	// get index into 2-level page table and initialize p
	...
	// p invalid on swap
			...
			// get the swap_offset and use new frame number
	...
	// p invalid and not on swap
	else {
			// check if first time reference
			if ((p->frame & PG_REF) == 0) {
			...
			p->frame = frame_num << PAGE_SHIFT;
			// set the page to be dirty if first time reference
			if (flag2 == 1) {
				p->frame = p->frame | PG_DIRTY;
			}
			miss_count++;
