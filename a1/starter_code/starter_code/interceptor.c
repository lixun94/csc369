...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	if(table[sysc].intercepted == 1){
		...
		//syscall is not monitored.
	}

	return table[syscall].f(reg);; // Just a placeholder, so it compiles with no warnings!
}
